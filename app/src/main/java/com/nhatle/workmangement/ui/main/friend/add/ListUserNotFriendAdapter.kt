...
class ListUserNotFriendAdapter(val call: SendDataProfile.FriendAdd) :
    ...
    override fun onBindViewHolder(holder: UserNotFriendHolder, position: Int) {
        ...
        registerImageAvatarClick(
            holder.itemView.imageAvatarFriend,
            getData()[position].profileId
        )
    }

    private fun registerImageAvatarClick(imageAvatarFriend: CircleImageView, profileId: Int) {
        imageAvatarFriend.setOnClickListener {
            call.sendData(profileId)
        }
    }
