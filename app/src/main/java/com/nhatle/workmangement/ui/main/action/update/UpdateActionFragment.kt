...
class UpdateActionFragment : BaseFragment(), UpdateActionContract.View, View.OnClickListener {
    ...
    private var actionSmall: ActionSmall? = null
    ...
    private val adapter: ActionSmallForUpdateAdapter by lazy {
        ActionSmallForUpdateAdapter(object : ActionSmallForUpdateAdapter.SendActionSmall {
            override fun sendActionSmall(
                data: ActionSmall,
                position: Int
            ) {
                this@UpdateActionFragment.position = position
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.buttonSaveUpdate -> {
                ...
                if (checkItemClick && actionSmall != null) {

                    val actionSmall = ActionSmall(
                        actionSmall!!.actionSmallId,
                        actionSmall!!.actionId,
                        editActionSmallNameOnUpdate.text.toString()
                    )
                    adapter.replaceActionSmall(
                        position = position,
                        actionSmall = actionSmall
                    )
                    listUpdate.add(actionSmall)
                }
    ...
    private fun updateWork() {
        if (editDescription.text.toString().isNotEmpty() || nameWork.text.toString().isNotEmpty()) {
            ...
            if (listUpdate.isNotEmpty()) {
                for (actionSmallUpdate in listUpdate){
                    presenter!!.updateActionSmall(actionSmallUpdate)
                }
            }
