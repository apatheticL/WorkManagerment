...
class UserProfileRemoteRepository(private val dataSource: UserProfileRemoteDataSource):UserProfileDataSource.Remote {
    override fun login(userName: String, password: String,callback: OnDataLoadedCallback<UserProfile>) {
        dataSource.login(userName,password,callback)
    }

    override fun registerAccount(registerResponse: RegisterResponse,callback: OnDataLoadedCallback<UserProfile>) {
